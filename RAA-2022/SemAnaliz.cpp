		...
		for (int i = 0; i < tables.lexTable.size; i++)
		{
			...
			case LEX_EQUAL: {
				...
				if (tables.lexTable.table[position - 1].lexema == LEX_ID)
				{
					...
					{
						if (tables.lexTable.table[position].lexema == LEX_ID || tables.lexTable.table[position].lexema == LEX_LITERAL|| tables.lexTable.table[position].lexema == LEX_COMPARE)
						{
							if (tables.idTable.table[tables.lexTable.table[position].idxTI].idType == IT::IDTYPE::F|| tables.lexTable.table[position].lexema == LEX_COMPARE)
							{
								int counter = 0;
