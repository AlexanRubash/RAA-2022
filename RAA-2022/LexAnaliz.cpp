...
namespace Lex
{
	LEX lexAnaliz(Log::LOG log, In::IN in)
	{
		...
		for (int i = 0; word[i] != NULL; i++, indexLex++) 																																																												//проход по всем лексемам
		{
			...
			{
				unsigned char powfunc[8] = { 'm', 'a', 'i', 'n', 'p', 'o', 'w', '\0' };
				unsigned char comparefunc[12] = { 'm', 'a', 'i', 'n', 'c', 'o', 'm','p','a','r','e', '\0' };
				unsigned char powfunc2[4] = {  'p', 'o', 'w', '\0' };
				unsigned char comparefunc2[8] = { 'c', 'o', 'm','p','a','r','e', '\0' };
				if (lexTable.table[lexTable.size - 2].lexema == LEX_VAR && lexTable.table[lexTable.size - 1].lexema == 't') {
					if(_mbscmp(powfunc2, word[i]) == 0 || _mbscmp(comparefunc2, word[i]) == 0|| _mbscmp(powfunc, word[i]) == 0 || _mbscmp(comparefunc, word[i]) == 0)
				...
				else
				{
					...
					bool flag = false;
					if (_mbscmp(powfunc, word[i]) == 0|| _mbscmp(comparefunc, word[i]) == 0) flag = true;
					if (idx != TI_NULLIDX && !flag)																																																																	 																																																				// если такой идентификатор уже есть
					{
						if (lexTable.table[indexLex - 2].lexema == LEX_VAR)
							throw ERROR_THROW_IN(200, line, 0);
						LT::Entry entryLT;
			...
			if (FST::execute(fstLiteralInt) || FST::execute(fstLiteralBin))
			{
				...
				entryIT.value.vushort = (unsigned short)value;
